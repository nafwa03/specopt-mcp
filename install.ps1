# specopt-mcp install script (Windows)
...
Write-Host "=== Step 1/2: Installing specopt-server + DSPy ==="
pip install -e .

Write-Host "=== Step 2/2: Upgrading gepa to >=0.1.1 (overriding DSPy's pin) ==="
pip install 'gepa>=0.1.1,<0.2' --force-reinstall --no-deps

Write-Host "=== Done! ==="
Write-Host "Run 'python agent_brain.py' to start the agent."
